...
/* Intercambia.
El programa intercambia las columnas e un arreglo bidimensional. Los
...
void intercambio(float [][MAX], int, int);        /* Prorotipos de funciones. */
...
while (C > MAX || C < 1);   /* Se verifica que el numero de columnas sea
                            correcto. */
lectura(MAT, F, C);
intercambia(MAT, F, C);
imprime(MAT, F, C);
...
/* Esta funcion se utiliza para intercambiar las columnas del arreglo bidimensional.
Observa que el indice correspondiente a las columnas solo se mueve hasta la mitad
del arreglo. */
{
int I, J;
float AUX;
...
for (I=0; I< F; I++)
    for (J=0; J<C; J++)
            printf("\nEl elemento %d %d: %5.2f", I+1, J+1, A[I][J]);
}
