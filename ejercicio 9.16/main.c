...
void sumypro(FILE *);     /* Prototipos de funcion. */
...
while (!feof (ap1))
{
    ...
    r = atof(cad);
    /* Recuerdaque la funcion atof convierte una cadena de caracteres que
...
printf("\nSuma: %.2f", sum);
if (i)   /* Si el valor de i es distinto dxe cero, calcula el promedio. */
    printf("\nPromedio: %.2f", sum/i);
}
