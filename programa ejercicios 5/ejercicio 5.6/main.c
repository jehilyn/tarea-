...
const int MAX = 100;
/* MAX se utiliza para reservar el espacio maxima que pondra ocupar el arreglo. */
...
Lectura(VEC, MAX);
RES = Suma(VEC, MAX);
/* Se llama a la funcion Suma y se alamacena el resultado en la variable RES. */
printf("\n\nSuma del arreglo: %.2lf", RES);
}

void Lectura(float A[], int T)
/* La funcion Lectura se utiliza para leer u  arreglo unidimensional de T
